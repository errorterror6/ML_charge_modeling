...
class RNN(nn.Module):
    ...
    def train(self, n_epochs, model_params=parameters.model_params, dataset=parameters.dataset, records=parameters.records):
        ...
        """
        # Initialize dataset
        data = shjnn.data.CustomDataset(dataset['trajs'], dataset['times'])
        ...
        # val_loader = DataLoader(val_dataset, batch_size=model_params['n_batch'], shuffle=False, drop_last=True)
    ...
    def eval(model_params=parameters.model_params, dataset=parameters.dataset):
        return
        dataset = shjnn.data.CustomDataset(dataset['trajs'], dataset['times'])
        test_dataset = dataset

        # initialise testing data loader for random mini-batches
        #TODO: look at shjnn.dataLoader more.
        # DataLoader should be imported from torch.utils.data, not shjnn
        from torch.utils.data import DataLoader
        test_loader = DataLoader(test_dataset, batch_size = model_params['n_batch'], shuffle = True, drop_last = True)
        n = len(test_dataset)
