...
def make_train_step(func, rec, dec, optim, device, noise_std = 0.3, beta = 1.0):
    ...
    def train_step(traj, time):
        ...
        #print('time ', time.size(), time)
        #print('time 2 ', time.squeeze().size())

        #print(time.shape, time[0,...].squeeze())
        ...
        pred_z = odeint(func, z0, time[0,...].squeeze(), atol = atol).permute(1, 0, 2)
        ...
        #print('loss_kl', analytic_kl.shape, analytic_kl)


        # calculate total loss
        #loss = torch.sum(-logpx + beta * analytic_kl)
        loss = torch.mean(-logpx + beta * analytic_kl)
        #loss = -logpx + beta * analytic_kl

        # average loss over batch
        #loss = torch.sum(loss).unsqueeze(-1)

        #print('loss', loss, loss.shape)
        #print('loss_', loss.shape, loss)
        ...
        return loss.item(), torch.mean(logpx).detach().numpy(), torch.mean(analytic_kl).detach().numpy()
